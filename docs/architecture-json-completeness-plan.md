...
**Status:** In progress (2026-08-06) — Point A ✅ and Point B (detect + repair) ✅ implemented + unit-tested (16/16). Point B **is** Change 2 in `frontend-fix-loop-convergence-plan.md`.
...
- **Point B** (gen-time): `ImportClosureChecker` reads the real generated frontend imports and finds unresolved local modules (catches modules invented only at generation, which plan-time cannot see); `MissingModuleSynthesizer` **repairs** them with permissive placeholders (default + named exports, children passed through) so imports resolve. Wired into `FrontendValidationNode` after the Change-3 path fixers: **default repairs** (synthesize, rebuild, proceed); strict (`worker.completeness.import-closure-strict`) fails loud instead.
- **Pending:** the "targeted LLM gen" route for a non-layout miss (Point A specs the planned/prose cases; the deterministic stub covers gen-time inventions), and backend `OrderItemResponse` DTO synthesis.
**Date:** 2026-08-05
...
Wired into `FrontendValidationNode` **after** the Change-3 path fixers (so a remaining miss is genuinely absent, not mis-pathed) and **before** the ErrorFixAgent. **Default repairs** (synthesize placeholders, rebuild, proceed; writes `docs/IMPORT_CLOSURE_REPORT.md`); `worker.completeness.import-closure-strict=true` instead **fails loud** without stubbing (`WorkerException(CODE)`). Frontend-only by design: backend closure is already enforced by `javac`. **Open:** the "targeted LLM gen" route for a non-layout miss (deferred — Point A specs the planned/prose cases; the deterministic stub covers gen-time inventions), and backend `OrderItemResponse` DTO synthesis.
