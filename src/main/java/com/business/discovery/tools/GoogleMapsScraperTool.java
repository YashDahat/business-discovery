...
public class GoogleMapsScraperTool {
    ...
    @Tool("""
        ...
        """)
    public String scrapeBusinesses(GoogleMapsScraperJobRequest request) {
        log.info("[TOOL CALLED] scrapeBusinesses — keyword: {}", request.getKeywords());

        try {
            // runId is null for now — will be wired in properly in AgentRun phase
            List<BusinessEntity> businesses = scraperService.scrapeAndPersist(request, null);

            if (businesses.isEmpty()) {
                return "No businesses found for keyword: " + request.getKeywords();
            }
            ...
            return ("Successfully scraped and saved %d businesses for '%s':\n\n%s")
                    .formatted(businesses.size(), request.getKeywords(), summary);

        } catch (Exception e) {
            log.error("[TOOL ERROR] scrapeBusinesses failed for keyword: {}", request.getKeywords(), e);
            return "Scraping failed for keyword: " + request.getKeywords() + " — Error: " + e.getMessage();
        }
    }
}