...
public class ResearchTechStackNode implements NodeAction<ArchitectAgentState> {

    private static final String NODE_NAME = "RESEARCH_TECH_STACK";
    ...
    public Map<String, Object> apply(ArchitectAgentState state) {
        ...
        try {
            log.info("[{}] Researching tech stack for: {}", runId, category);
            String research = researchService.researchTechStack(category);

            long duration = System.currentTimeMillis() - start;
            eventService.stepCompleted(runId, NODE_NAME, duration);

            return Map.of(ArchitectAgentState.TECH_STACK_RESEARCH, research);
