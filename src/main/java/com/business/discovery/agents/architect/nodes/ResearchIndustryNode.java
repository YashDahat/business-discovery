...
public class ResearchIndustryNode implements NodeAction<ArchitectAgentState> {

    private static final String NODE_NAME = "RESEARCH_INDUSTRY";
    ...
    public Map<String, Object> apply(ArchitectAgentState state) {
        ...
        try {
            log.info("[{}] Researching industry: {} in {}", runId, category, location);
            String research = researchService.researchBusinessCategory(category, location);

            long duration = System.currentTimeMillis() - start;
            eventService.stepCompleted(runId, NODE_NAME, duration);

            return Map.of(ArchitectAgentState.INDUSTRY_RESEARCH, research);
