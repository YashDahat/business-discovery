...
class FrontendContractCardTest {
    ...
    @Test
    void prefersAuthoritativeHookContractSidecarOverRegexScan() throws Exception {
        // Task 11: a derived hook file carries `// @hook-contract …`. The card must use it verbatim
        // and NOT fall back to the lossy HOOK_DECL scan. Here the function omits its return annotation,
        // so the regex (Gap-A) could only recover the field NAMES from the return literal — never the
        // typed `ProductDto[] | undefined`. Its presence proves the sidecar won.
        write("frontend/src/hooks/productHooks.ts", """
                ...
                export function useProducts() {
                  const { data, isLoading, isError, error } = useQuery({ queryKey: ['product'], queryFn: getAllProducts });
        ...
        assertThat(section).contains(
                "useProducts(): { data: ProductDto[] | undefined; isLoading: boolean; isError: boolean; error: Error | null }");
    }
